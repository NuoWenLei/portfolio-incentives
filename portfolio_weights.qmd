...
Numerai currently evaluates participant submissions against the live market target using a method called MMC or Meta Model Contribution. This method first neutralizes all individual submissions to the Meta Model, and then calculates the covariance between the live market target and each participant's neutralized submission. The idea is that a higher covariance represents a stronger unique positive contribution to the Meta Model, and therefore should be rewarded more. [Should we talk about flaws?]
